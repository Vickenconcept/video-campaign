<x-app-layout>
    <div class=" bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 py-8 overflow-y-auto pb-40 h-screen">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                    <div>
                        ...
                        <p class="text-lg text-slate-600">
                            Email campaign details and recipient analytics
                        </p>
                    </div>
                    
                    <!-- Enhanced Action Buttons -->
                    <div class="flex flex-wrap gap-3">
                        <a href="{{ route('email.campaigns.templates', $campaign) }}" 
                            class="inline-flex items-center gap-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 font-semibold">
                            <i class='bx bx-paint text-xl'></i>
                            Change Template
                        </a>
                        
                        <a href="{{ route('email.campaigns.embed', $campaign) }}" 
                            class="inline-flex items-center gap-3 bg-gradient-to-r from-indigo-700 to-blue-700 hover:from-indigo-800 hover:to-blue-800 text-white px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 font-semibold">
                            <i class='bx bx-code-alt text-xl'></i>
                            Get Embed HTML
                        </a>
                        
                        <a href="{{ route('email.campaigns.edit', $campaign) }}" 
                            class="inline-flex items-center gap-3 bg-gradient-to-r from-indigo-600 to-blue-700 hover:from-indigo-700 hover:to-blue-800 text-white px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 font-semibold">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                                <i class='bx bx-palette text-slate-500'></i>
                                <span class="text-slate-700"><span class="font-semibold">Template:</span> 
                                    <span class="capitalize px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-sm font-medium">{{ $campaign->template }}</span>
                                </span>
                            </div>
                            <div class="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div class="bg-gradient-to-r from-amber-50 to-yellow-50 p-4 rounded-xl border border-amber-200">
                                <div class="text-2xl font-bold text-amber-700">{{ $stats['total_recipients'] ? round($stats['opened'] / max(1, $stats['total_recipients']) * 100, 1) : 0 }}%</div>
                                <div class="text-sm text-amber-600">Open Rate</div>
                            </div>
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl shadow-slate-500/10 border border-white/20 p-8 mb-8">
                ...
                @if($campaign->video_url)
                    <div class="bg-gradient-to-r from-slate-50 to-blue-50 rounded-xl p-6 border border-slate-200">
                        <h3 class="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
                            <i class='bx bx-video text-indigo-600'></i>
                            Campaign Video
                        </h3>
                        ...
                        <div class="flex flex-col lg:flex-row items-start gap-6">
                                ...
                                @if(str_contains($campaign->video_url, 'youtube.com') || str_contains($campaign->video_url, 'youtu.be'))
                                    ...
                                    @else
                                        <a href="{{ $campaign->video_url }}" target="_blank" class="text-blue-600 underline">
                                            <img src="{{ asset('images/video-thumbnail.jpg') }}" alt="Thumbnail" class="w-52 object-cover rounded-xl shadow-lg">
                                        </a>
                                    @endif
                            ...
                            <div class="flex flex-col gap-3">
                                <a href="{{ $campaign->video_url }}" target="_blank" 
                                    class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200 font-semibold">
                                    <i class='bx bx-external-link text-xl'></i>
                                    View Video
                                </a>
                                <p class="text-sm text-slate-600 max-w-md">
                                    Click the button above to view the full video content associated with this campaign.
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <thead>
                            <tr class="border-b-2 border-slate-200">
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider bg-slate-50">
                                    <i class='bx bx-envelope-open mr-2'></i>Opened
                                </th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider bg-slate-50">
                            ...
                            @foreach($recipients as $recipient)
                                <tr class="hover:bg-slate-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 text-slate-600">
                                        {{ $recipient->opened_at ? \Illuminate\Support\Carbon::parse($recipient->opened_at)->format('M j, Y g:i A') : '-' }}
                                    </td>
                                    <td class="px-6 py-4 text-slate-600">
